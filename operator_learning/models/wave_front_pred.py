...
class FrontSpeedPredictor(nn.Module):
    """
    Neural network that predicts front speeds and rarefaction confidence from uL/uR values.
    
    Takes left and right state values at a discontinuity and outputs:
    - front1_speed: speed of the first (or only) front
    - front2_speed: speed of the second front (for rarefactions)
    - front2_confidence: probability that this is a rarefaction (two fronts) vs shock (one front)
    """
    ...
    def __init__(self, hidden_dim: int):
        ...
        # Output head: predicts (front1_speed, front2_speed, front2_confidence)
    ...
    def forward(self, uL: torch.Tensor, uR: torch.Tensor) -> torch.Tensor:
        """
        Predict front speeds and confidence from left/right state values.
        ...
        Returns:
            front_params: Shape (..., 3) with channels:
                          [front1_speed, front2_speed, front2_confidence]
                          Note: front2_confidence has sigmoid applied (0-1 range)
        """
        ...
        output = self.output_head(h)   # (..., 3)
        
        # Extract raw outputs before modification
        front1_speed = output[..., 0:1]  # (..., 1)
        raw_front2_speed = output[..., 1:2]  # (..., 1) - raw prediction
        confidence = torch.sigmoid(output[..., 2:3])  # (..., 1) - front2_confidence with sigmoid
        
        # Enforce front2_speed >= front1_speed: front2_speed = front1_speed + abs(raw_front2_speed)
        front2_speed = front1_speed + torch.abs(raw_front2_speed)  # (..., 1)
        
        # Concatenate: [front1_speed, front2_speed, front2_confidence]
        output = torch.cat([front1_speed, front2_speed, confidence], dim=-1)
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            front_params: (B, 6, max_fronts) where the 6 channels are:
                          (front1_speed, front2_speed, front2_confidence, uL, uR, front_coords)
                          Sorted by front_coords (left to right).
                          Invalid fronts (beyond actual count) have confidence = -1.
        ...
        has_discontinuity = (left_vals - right_vals).abs() > self.diff_threshold  # (B, X-1)
        ...
        _, topk_indices = torch.topk(selection_score, k=min(K, X-1), dim=-1, largest=False)  # (B, K)
        ...
        front_coords = torch.gather(all_front_coords, dim=-1, index=topk_indices)  # (B, K)
        is_valid = torch.gather(has_discontinuity, dim=-1, index=topk_indices)  # (B, K)
        
        # Use FrontSpeedPredictor to get speeds and confidence
        speed_params = self.speed_predictor(uL, uR)  # (B, K, 3)
        
        # Mask invalid fronts - avoid in-place ops for gradient flow
        mask = is_valid.unsqueeze(-1).float()  # (B, K, 1)
        
        # Mask speeds (zero for invalid fronts)
        masked_speeds = speed_params[:, :, :2] * mask  # (B, K, 2)
        
        # Mask confidence (-1 for invalid fronts)
        masked_conf = torch.where(
            is_valid.unsqueeze(-1),
            speed_params[:, :, 2:3],
            torch.full_like(speed_params[:, :, 2:3], -1.0)
        )  # (B, K, 1)
        
        # Concatenate back together
        front_params = torch.cat([masked_speeds, masked_conf], dim=-1)  # (B, K, 3)
        
        # Transpose to (B, 3, K)
        front_params = front_params.permute(0, 2, 1)
        
        # Concatenate all channels: (B, 6, K)
        front_params = torch.cat([
            front_params,                          # (B, 3, K): speeds and confidence
...
class WavePredictor(nn.Module):
    ...
    def forward(self, ul, ur, coord, x):
        ...
        # Replace first channel at t=0 with Riemann IC
        model_input[:, 0, 0, :] = riemann_ic_t0  # Only modify t=0
...
class RegionIndexer(nn.Module):
    def __init__(self, front_threshold: float = 0.5, sharpness: float = 50.0):
        super().__init__()
    ...
    def forward(self, front_params: torch.Tensor, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        Indexing logic based on front2_confidence:
        - front2_conf == -1: Not a discontinuity, skip entirely
        - front2_conf < threshold: Shock (single front), crossing adds +2 (even → even)
        - front2_conf >= threshold: Rarefaction (two fronts), each crossing adds +1
        ...
        front_coords = front_params[:, 5, :]      # (B, K) - initial x position
        
        # Soft masks for different front types (differentiable)
        # Use sigmoid for soft thresholding
        is_valid_soft = torch.sigmoid((front2_conf + 0.5) * 10)  # Soft version of conf > -0.5
        is_rarefaction_soft = torch.sigmoid((front2_conf - self.front_threshold) * 10)  # Soft version of conf >= threshold
        is_shock_soft = is_valid_soft * (1 - is_rarefaction_soft)  # Valid but not rarefaction
        ...
        front1_pos = front_coords.unsqueeze(2) + front1_speed.unsqueeze(2) * t_unique.unsqueeze(1)
        front2_pos = front_coords.unsqueeze(2) + front2_speed.unsqueeze(2) * t_unique.unsqueeze(1)
        ...
        is_rarefaction_flat = is_rarefaction_soft.unsqueeze(1).expand(B, T, K).reshape(B * T, 1, K)
        ...
        right_of_front1_soft = torch.sigmoid(self.sharpness * (x_flat - front1_flat))  # (B*T, X, K)
        right_of_front2_soft = torch.sigmoid(self.sharpness * (x_flat - front2_flat))  # (B*T, X, K)
        
        # For shocks: crossing front1 adds +2
        shock_contribution = (right_of_front1_soft * is_shock_flat * 2).sum(dim=-1)  # (B*T, X)
        
        # For rarefactions: crossing front1 adds +1, crossing front2 adds +1
        rarefaction_contribution = (
            (right_of_front1_soft * is_rarefaction_flat) + 
            (right_of_front2_soft * is_rarefaction_flat)
        ).sum(dim=-1)  # (B*T, X)
...
class WaveFront(nn.Module):
    def __init__(self, 
        ...
        super().__init__()
        self.max_fronts = max_fronts
        self.wave_predictor = WavePredictor(n_modes, hidden_dim, in_channels, out_channels, n_layers)
        self.front_detector = FrontDetector(hidden_dim, max_fronts=max_fronts)
        self.region_indexer = RegionIndexer(front_threshold)
        self.front_threshold = front_threshold
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # front_params: (B, 6, K), front_count: (B,)
...
if __name__ == "__main__":
    ...
    print(f"Front params shape: {front_params.shape}")  # Should be (B, 6, max_fronts)
    print(f"  - Expected: ({B}, 6, {max_fronts})")
    print(f"Front count per sample: {front_count.tolist()}")
    
    # Check discontinuity detection
    conf = front_params[:, 2, :]  # confidence channel
    n_valid = (conf > -0.5).sum(dim=-1)
    print(f"Valid fronts per sample (from confidence): {n_valid.tolist()}")
    
    # Test RegionIndexer
    print("\n--- Testing RegionIndexer ---")
    indexer = RegionIndexer(front_threshold=0.5).to(device)
