...
def solve_batch(
    ...
    *,
    ...
    for step in range(nt):
        ...
        if use_weno:
            # SSP-RK3
            k1_rho, k1_rho_u, k1_E = _compute_rhs_batch(rho, rho_u, E)
            rho_1 = (rho + dt * k1_rho).clamp(min=0.0)
            rho_u_1 = rho_u + dt * k1_rho_u
            E_1 = E + dt * k1_E
            ...
            new_rho_u = (1 / 3) * rho_u + (2 / 3) * (rho_u_2 + dt * k3_rho_u)
            new_E = (1 / 3) * E + (2 / 3) * (E_2 + dt * k3_E)
        else:
            # Forward Euler
            k1_rho, k1_rho_u, k1_E = _compute_rhs_batch(rho, rho_u, E)
            new_rho = (rho + dt * k1_rho).clamp(min=0.0)
            new_rho_u = rho_u + dt * k1_rho_u
            new_E = E + dt * k1_E

        # Freeze dead samples at their last valid state
        rho = torch.where(alive_exp, new_rho, rho)
        rho_u = torch.where(alive_exp, new_rho_u, rho_u)
