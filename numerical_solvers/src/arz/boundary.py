...
def apply_ghost_cells(
    ...
    *,
    ...
    def _dirichlet_ghost(
        ...
        ghost_shape = (*rho.shape[:-1], ng)
        rho_g = torch.full(ghost_shape, rho_val, device=device, dtype=dtype)
        rw_g = torch.full(ghost_shape, rw_val, device=device, dtype=dtype)
        return rho_g, rw_g

    if bc_type == "periodic":
        rho_g = torch.cat([rho[..., -ng:], rho, rho[..., :ng]], dim=-1)
        rho_w_g = torch.cat([rho_w[..., -ng:], rho_w, rho_w[..., :ng]], dim=-1)
        ...
        rho_gr = rho[..., -1:].expand(*rho.shape[:-1], ng)
        rw_gr = rho_w[..., -1:].expand(*rho_w.shape[:-1], ng)
        rho_g = torch.cat([rho_gl, rho, rho_gr], dim=-1)
        rho_w_g = torch.cat([rw_gl, rho_w, rw_gr], dim=-1)
        ...
        rho_w_g = torch.cat(
            [rho_w[..., :1].expand(*rho_w.shape[:-1], ng), rho_w, rho_w[..., -1:].expand(*rho_w.shape[:-1], ng)],
            dim=-1,
        )
