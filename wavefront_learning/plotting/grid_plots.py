...
_EULER_CHANNELS = [
    ("rho", "Density (rho)", "viridis"),
    ("u", "Velocity (u)", "plasma"),
    ("p", "Pressure (p)", "inferno"),
]


def plot_euler_ground_truth(
    traj_data: dict,
    grid_config: dict,
) -> list[tuple[str, Figure]]:
    """Plot Euler ground truth with rho, u, and p side by side.

    Args:
        traj_data: Dict containing 'grids' of shape (B, 3, nt, nx).
        grid_config: Dict with {nx, nt, dx, dt}.
    ...
    for b in range(min(B, 3)):
        ...
        for c, (name, label, cmap) in enumerate(_EULER_CHANNELS):
            im = axes[c].imshow(
                grids[b, c],
                extent=extent,
                aspect="auto",
            ...
            plt.colorbar(im, ax=axes[c], label=label)
        plt.tight_layout()
        figures.append((f"euler_gt_sample_{b + 1}", fig))
...
def plot_euler_pred(
    ...
    """Plot Euler predictions with rho, u, and p side by side.

    Args:
        traj_data: Dict containing 'output_grid' of shape (B, 3, nt, nx).
        grid_config: Dict with {nx, nt, dx, dt}.
    ...
    for b in range(min(B, 3)):
        ...
        for c, (name, label, cmap) in enumerate(_EULER_CHANNELS):
            ...
            plt.colorbar(im, ax=axes[c], label=label)
        plt.tight_layout()
        figures.append((f"euler_pred_sample_{b + 1}", fig))

    return figures


def plot_euler_mse_error(
    traj_data: dict,
    grid_config: dict,
) -> list[tuple[str, Figure]]:
    """Plot per-channel MSE error for Euler (rho, u, p) side by side.

    Args:
        traj_data: Dict containing 'output_grid' and 'grids' of shape (B, 3, nt, nx).
        grid_config: Dict with {nx, nt, dx, dt}.
    ...
    for b in range(min(B, 3)):
        fig, axes = plt.subplots(1, 3, figsize=(21, 6))
        for c, (name, label, _cmap) in enumerate(_EULER_CHANNELS):
            channel_error = (output_grid[b, c] - grids[b, c]) ** 2
            im = axes[c].imshow(
                channel_error,
                extent=extent,
            ...
            plt.colorbar(im, ax=axes[c], label=f"MSE ({label})")
        plt.tight_layout()
        figures.append((f"euler_mse_sample_{b + 1}", fig))
