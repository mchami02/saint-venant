"""WaveNOFull: Full Wavefront Neural Operator for hyperbolic conservation laws.
...
WaveNOFull lets spatial queries discover the relevant segment information via
...
class WaveNOFull(nn.Module):
    """Full Wavefront Neural Operator.
    ...
    def _forward_seg(
        ...
        """Segment-based forward pass (original WaveNOFull)."""
        xs = batch_input["xs"]
        ks = batch_input["ks"]
    ...
    def _forward_disc(
        ...
        """Discontinuity-based forward pass (WaveNOFullDisc).
        ...
        if len(self.cross_segment_layers) > 0:
            ...
            cs_mask = (
                (~disc_mask.bool()).unsqueeze(1).expand(B, nt, D).reshape(B * nt, D)
            )
            all_masked_cs = cs_mask.all(dim=1)
            if all_masked_cs.any():
                cs_mask = cs_mask.clone()
        ...
        if self.predict_trajectories:
            time_emb = self.traj_time_encoder(t_unique)  # (B, nt, H)
            positions = self.traj_decoder(emb, time_emb, disc_mask)  # (B, D, nt)
...
def build_waveno_full(args: dict) -> WaveNOFull:
    """Factory function for WaveNOFull.

    Args:
        args: Dict or Namespace with model configuration.

    Returns:
        Configured WaveNOFull instance.
    """
...
def _build_waveno_full_base(args: dict, **overrides) -> WaveNOFull:
    """Shared builder for WaveNOFull variants."""
    if not isinstance(args, dict):
        args = vars(args)
    kwargs = dict(
        hidden_dim=args.get("hidden_dim", 64),
    ...
    return WaveNOFull(**kwargs)


def build_waveno_full_cls(args: dict) -> WaveNOFull:
    """WaveNOFull + classifier head to filter breakpoints."""
    return _build_waveno_full_base(args, with_classifier=True)


def build_waveno_full_local(args: dict) -> WaveNOFull:
    """WaveNOFull without cumulative mass (N_k) in segment encoder."""
    return _build_waveno_full_base(args, local_features=False)


def build_waveno_full_indep_traj(args: dict) -> WaveNOFull:
    """WaveNOFull with independent trajectory decoding from raw discontinuities."""
    return _build_waveno_full_base(args, independent_traj=True)


def build_waveno_full_disc(args: dict) -> WaveNOFull:
    """WaveNOFull with discontinuity-based tokens instead of segments."""
    return _build_waveno_full_base(args, use_discontinuities=True)


def build_waveno_full_base(args: dict) -> WaveNOFull:
    """WaveNOFull without trajectory prediction (grid-only output)."""
    return _build_waveno_full_base(args, predict_trajectories=False)


def build_shock_aware_waveno_full(args: dict) -> WaveNOFull:
    """WaveNOFull with shock proximity prediction head."""
    return _build_waveno_full_base(args, predict_proximity=True)
