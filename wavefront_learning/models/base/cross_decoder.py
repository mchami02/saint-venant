...
class CrossDecoderLayer(nn.Module):
    ...
    def forward(self, x, z, key_padding_mask=None):
        ...
        att = self.cross_attention(
            x_norm, z_norm, z_norm, key_padding_mask=key_padding_mask
        )[0]
        x = x + att
...
class CrossDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.grid_features = nn.Parameter(
            torch.randn(grid_nx, grid_ny, hidden_dim) * 0.02
        )
    ...
    def interpolate_features(self, query_points):
        ...
        diff = query_points.unsqueeze(2) - self.grid_points.unsqueeze(0).unsqueeze(
            0
        )  # (B, Q, G, 2)
    ...
    def forward(self, coords, encoder_output, key_padding_mask=None):
        ...
        Returns:
            output: (B, T, N, output_dim) - decoded output at query points
        """
