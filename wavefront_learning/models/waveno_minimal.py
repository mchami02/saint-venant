...
class WaveNOMinimal(nn.Module):
    ...
    def forward(
        ...
        if self.use_film:
            ...
            kv = self.time_conditioner(seg_emb, t_unique)  # (B, nt, S, H)
        else:
            kv = seg_emb.unsqueeze(1).expand(B, nt, S, self.hidden_dim)

        kv = kv.reshape(B * nt, S, self.hidden_dim)  # (B*nt, S, H)
        ...
        if self.use_cross_seg_attn:
            ...
            kv = kv.reshape(B, nt, S, self.hidden_dim)
            kv = kv * segments_mask.unsqueeze(1).unsqueeze(-1)  # re-zero
            kv = kv.reshape(B * nt, S, self.hidden_dim)
